...
void setup(){
  ...
  out = SD.open("synthatria.txt", FILE_WRITE);
  if (out){
...
void pulse(){
  ...
  while (millis() - start < 2){
    print_pulse(1);
    out.print(micros()); out.println(" -1");
  }
  digitalWrite(13, LOW);
...
void loop(){
  for (int i = 0; i < 256; i++){
    ...
    } else if (micros() - t > thresh_p*heart_micros) {
      t = micros();
      pulse();
      arr_pulsed = true;
      found = true;
  ...
  if (!arr_pulsed){
    ...
    out.print(micros()); out.print(" "); out.println(f);
    if (f > 3.5) {
      //fix_tachycardia();
      pulse_mode = true;
    } else {
